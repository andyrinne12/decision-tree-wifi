...
def print_metrics(metrics):
    ...
    df1 = pd.DataFrame.from_dict(dict(
        ("Room " + str(i + 1), [p[i], r[i], f[i], None]) for i in range(p.shape[0])), orient="index")
    df1 = df1.append(pd.Series([p_macro, r_macro, f_macro, a], name="Macro"))
    df1 = df1.set_axis(["Precission", "Recall", "F1",
                       "Accuracy"], axis=1, inplace=False)
    return df1
